...
type Robotization = 1 | 2 | 3 | 4;
type Entry = HTMLInputElement | HTMLTextAreaElement;
...
interface Music {
	...
	soprano: Array<string> | undefined,
	contralto: Array<string> | undefined,
	tenor: Array<string> | undefined,
	bass: Array<string> | undefined,
}
...
class Synthesizer {
	...
	static types = ['sine', 'triangle', 'sawtooth', 'square'] as Array<OscillatorType>;
	static staccato = 20;

	constructor(type: number, volume: number = 0.3) {
...
async function playMelody(melody, bpm, robotization, delay = 500, loop = false) {
	...
	function play_sounds() {
		if (playedNotes < notes.length) {
			let note = notes[playedNotes] as keyof typeof frequencies;

			let noteExists = Boolean(Object.keys(frequencies).filter((key) => key === note)[0]);
			if (!noteExists) throw new Error('There is a note in your melody that does not exist');
	...
	if (delay) setTimeout(play_sounds, delay);
	else play_sounds();
...
class Entries {
	static getValue(selector: string) {
		let element = document.querySelector(selector) as Entry;
...
function getActualMusic() {
	...
	if (robotizationValue) {
		...
		if (!isRobotizationSupported) throw new Error('This robotization does not exist.');
	}
	...
	let music: Music = {
		...
		robotization: Number(Entries.getValue('.settings #music-robotization')) as Robotization || Falsy.number,
	...
	if (!music.name) music.name = 'Untitled';
	if (!music.bpm) music.bpm = 60;
	if (!music.robotization) music.robotization = 1;
...
function addMusicCard(name: string, id: number) {
...
function saveMusic() {
	...
	if (actualMusic) {
		for (let i = 0; i < localStorage.length; i++) {
			...
			if (storedKey) storedMusic = localStorage.getItem(storedKey);
...
function resetMusic(id: number) {
	let storedMusic; 
	let resetedMusic;

	storedMusic = localStorage.getItem(`MUSIC${id}`);
	if (storedMusic) resetedMusic = JSON.parse(storedMusic);

	if (storedMusic && resetedMusic) {
		resetedMusic = JSON.parse(resetedMusic) as Music;
...
function removeMusic(card: Element) {
	let newSavedMusics: Array<number> = [];

	if (card.parentElement) {
		for (let i = 0; i < savedMusics.length; i++) {
			if (!(savedMusics[i] === Number(card.parentElement.id))) newSavedMusics.push(savedMusics[i]);
		}
...
function playMusic(loop: boolean) {
	let music = getActualMusic();

	if (music) {
		if (!music.bpm) music.bpm = 60;
		if (!music.robotization) music.robotization = 2;
	
		if (music.soprano) playMelody(music.soprano, music.bpm, music.robotization, undefined, loop);
		if (music.contralto) playMelody(music.contralto, music.bpm, music.robotization, undefined, loop);
		if (music.tenor) playMelody(music.tenor, music.bpm, music.robotization, undefined, loop);
		if (music.bass) playMelody(music.bass, music.bpm, music.robotization, undefined, loop);
	}
}
